...
def dequeue_and_enqueue_HGPSL(hidden_feats_dict, start, end, queue):
    '''
    update memory bank by batch window slide; hidden_batch_feats must be normalized
    '''
    assert(hidden_feats_dict.keys() == queue.keys())

    for i in queue:
        queue[i][start:end] = nn.functional.normalize(hidden_feats_dict[i],dim=1)
    return queue
